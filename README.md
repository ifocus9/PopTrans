...
|           文本翻译工作台           |           划词 / OCR 结果弹窗           |            设置界面             |
| :--------------------------------: | :-------------------------------------: | :-----------------------------: |
| ![划词翻译结果](images/result.png) | ![文本翻译工作台](images/translate.png) | ![设置界面](images/setting.png) |
...
> _校验规则：端口超出范围、主题非 `system`/`light`/`dark`、空闲时间为负数时，对应字段会回退到默认值；缺失字段也会自动补全为默认值。修改快捷键请优先在设置界面操作，手动编辑表达式易因格式错误而无效。_
